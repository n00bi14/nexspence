...
| `auth.password_min_length` | `8` | Minimum length enforced on every locally-set password (user creation, self-service change in the profile modal, admin reset). LDAP / OIDC / SAML accounts have no local password and are excluded. |
